...
class RegisterExerciseViewModel {
    ...
    init(categoryId: String, name: String, series: Int, repetitions: Int) {
    ...
    public func registerExercise(completion: @escaping (ExerciseRequest?, Error?) -> Void ) {
        let exerciseRequest = ExerciseRequest(id: "", name: self.name, series: self.series, repetitions: self.repetitions)
